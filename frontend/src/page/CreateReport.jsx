...
function CreateReport({ reports }) {
  ...
  const handleCreateReport = async () => {
    ...
    // 선택된 title, image, content 값을 가져옵니다
    const selectedTitle = title[selectedTitles.findIndex(Boolean)];
    const selectedImage = imige[selectedImages.findIndex(Boolean)];
    const selectedContent = contents[selectedContents.findIndex(Boolean)];
   
    
  
    try {
      const res = await fetch('http://localhost:3000/createreport/createreport', {
        method: 'POST',
        headers: {
  ...
  const [previewContent, setPreviewContent] = useState(null); // 미리보기 내용을 저장하는 상태
  ...
  const handleMouseEnter = (index) => {
    // 마우스를 올린 후 1초 뒤에 미리보기 창 표시
    previewTimeout = setTimeout(() => {
      setPreviewContent(contents[index]); // 미리보기 창에 표시할 내용 설정
      setShowPreview(true); // 미리보기 창 표시
    }, 3000); // 1초 동안 마우스를 유지하면 미리보기 창 표시
  };

  const handleMouseLeave = () => {
    clearTimeout(previewTimeout); // 마우스가 떠나면 타이머 취소
    setShowPreview(false); // 미리보기 창 숨김
  };
  ...
  const handleTitleSelect = (index) => {
    handleSelect(index, setSelectedTitles, selectedTitles);
    
  };

  const handleImageSelect = (index) => {
    handleSelect(index, setSelectedImages, selectedImages);
    
  };

  const handleContentSelect = (index) => {
    handleSelect(index, setSelectedContents, selectedContents);
    
  };

  const isFormValid = selectedTitles.includes(true) && selectedImages.includes(true) && selectedContents.includes(true);

  return (
    <div className="create-report-container">
      {/* <span className="instruction-text">생성할 레포트의 요소를 골라주세요!</span> */}
      ...
      <div className="report-grid">
        ...
        {imige.map((imgSrc, index) => (
          <div
            key={index}
            className={`grid-item image ${selectedImages[index] ? 'selected' : ''}`}
            onClick={() => handleImageSelect(index)}
          >
            <img src={imgSrc} alt={`Report Image ${index + 1}`} className="image-content" />
            {selectedImages[index] && <div className="overlay"></div>} {/* 오버레이 추가 */}
          </div>
        ))}
        ...
        {contents.map((item, index) => (
          <div
            ...
            onMouseEnter={() => handleMouseEnter(index)} // 마우스를 올리면 실행
            onMouseLeave={handleMouseLeave} // 마우스를 떼면 실행
            onClick={() => handleContentSelect(index)}
          >
            {item} {/* 리스트의 content 표시 */}
          </div>
        ))}
      </div>
      ...
      <div className="create-report-button-container">
        <button
          ...
          onClick={handleCreateReport} // 버튼 클릭 시 데이터 전송 함수 실행
